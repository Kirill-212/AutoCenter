...
import PutCarsEquipment from "../../Services/CarEquipment/PutCarEquipmentService";
import GetFormCarEquipments from "../../Services/CarEquipment/GetFormCarEquipmentService";
import GetCarEquipments from "../../Services/CarEquipment/GetCarEquipmentByNameService";
import PostImgs from "../../Services/Img/ImgService";

const PutCarEquipment = () => {
  ...
  const [carEquipmentPut, setCarEquipmentPut] = React.useState([]);
  const [carEquipmentPutJson, setCarEquipmentPutJson] = React.useState([]);
  ...
  async function GetCarEquipment(name) {
    setMessageError("");
    let response = await GetCarEquipments(name);
    if (response === undefined) {
      setMessageError("Check connect server");
    } else {
      if (response.status !== 200) {
        ...
        setViewImg(response.data.urlImg);
        setimg(response.data.urlImg);
        setCarEquipmentPutJson(response.data.equipment);
        for (let i in response.data.equipment)
          carEquipmentPut.push(response.data.equipment[i]);
        setFlag(true);
      }
    }
  }
  ...
  function RenderRedioButton(input, name) {
    let flagRadioButton = false;
    let returnButtons = [];
    for (let j in input) {
      for (let i in carEquipmentPut) {
        if (
          carEquipmentPut[i].value === input[j].value &&
          carEquipmentPut[i].cost === input[j].cost
        ) {
  ...
  async function submitCarEquipment(event) {
    ...
    setMessageError("");
    if (failed) {
      setMessageError("Error:input file");
      return;
    }
    let count = 0;
    for (let i in carEquipmentList.equipmentItems) {
    ...
    if (carEquipment.length !== count) {
      setMessageError("Error:input radio button");
      return;
    }
    let arr = "{";
    ...
    let url = "";
    if (img !== viewImg) {
      url = await PostImgs.uploadImage(img);
    }
    let response = await PutCarsEquipment(name, JSON.parse(arr), url);
    if (response === undefined) {
      setMessageError("Check connect server");
    } else {
      if (response.status !== 200) {
  ...
  function onChangeValue(event) {
    ...
    rs = JSON.parse(rs);
    console.log("carEquipment[0]", carEquipment[0]);
  ...
  useEffect(() => {
    ...
    GetCarEquipment(query.get("name"));
    GetFormCarEquipment();
  }, []);
        ...
        <form onSubmit={submitCarEquipment}>
          ...
          {viewImg !== undefined && (
            <div className="col-md-4">
              <img src={viewImg} className="w-100 h-100 img-thumbnail" />
            </div>
          )}
          <div className="form-group mb-2 ">
            <label>Car equipment</label>
            {flag && flagForm && RenderCarEquipment()}
          </div>
